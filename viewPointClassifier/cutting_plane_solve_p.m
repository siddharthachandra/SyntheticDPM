function [parm] = cutting_plane_solve_p(parm,model),
	...
	if 0,	
		...
		f(end) = -1;
		for sample = 1 : parm.numSamples, %finding most violated constraints for each sample, and constructing the initial constraint. 
		...
		fprintf('INITIAL OBJECTIVE Function Value: %g\n',parm.c*(sum(f)/parm.numSamples));
	end

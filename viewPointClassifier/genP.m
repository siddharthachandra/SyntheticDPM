...
%dbstop if error;
coverage3 = zeros(parm.numSamples,parm.numParts);
for sample = 1 : parm.numSamples,
	...
	for part = 1 : parm.numParts,
		if any(part_im(:) == part),
			%indicatorImage = imresize(single(part_im==part),(parm.sizeRoot+2)*parm.sbin,'nearest');
			%score = conv2(indicatorImage, partTemplate, 'valid');
			%indicatorImage = imresize(single(part_im==part),(parm.sizeRoot+2)*parm.sbin,'nearest');
			score = conv2(single(part_im == part), partTemplate, 'valid');
			[v, YY] = max(score);
			[v, xp] = max(v);
			coverage3(sample,part) = v;
			if 0
			yp = YY(xp);
			xx = xp; yy=yp;
			%% changing 

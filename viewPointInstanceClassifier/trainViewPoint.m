...
for sample = 1 : parm.numSamples,
	...
	insts = union(insts,[y.instance]);
	Y{sample} = y;
end

parm.numViews = length(vps);
parm.numInstances = length(insts);

parm.dimension = parm.numInstances*parm.numViews*prod([parm.sizeRoot parm.featDim]);

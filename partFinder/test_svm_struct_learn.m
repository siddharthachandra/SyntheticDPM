...
function mypsi = featureCB(param, x, y)
  mypsi = sparse(y*x/2) ;
  if param.verbose
    fprintf('w = mypsi([%8.3f,%8.3f], %3d) = [%8.3f, %8.3f]\n', ...
            x, y, full(mypsi(1)), full(mypsi(2))) ;
  end
end

function yhat = constraintCB(param, model, x, y)
% slack resaling: argmax_y delta(yi, y) (1 + <mypsi(x,y), w> - <mypsi(x,yi), w>)
% margin rescaling: argmax_y delta(yi, y) + <mypsi(x,y), w>
